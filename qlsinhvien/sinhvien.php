<?php 
	...
	require "model/mdl_lopchuyennganh.php";
?>
...
<div class="group-box">
	...
	<form method="post" name="frmSV" action="<?php echo $_SERVER["PHP_SELF"];?>">		
		...
		<select name="MaLop" id="svDSLop" >
			...
			<?php
				// IN danh sách lớp
				$lop = new Lopchuyennganh();
				$jdata = $lop->getListClass();
				if ($jdata != "401"){
	...
	<!--  form này không có tác dụng gì, chỉ dùng để chỉ ra nút Sửa ở cột bên phải bảng thuộc về form này, 
	mục đích là không cho post dữ liệu dư thừa khi nhấn nút Sửa, vì chỉ cần giá trị trong thuộc tính value 
	của nút Sửa sử dụng tag <button>  -->
	</form>
